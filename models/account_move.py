...
class AccountMove(models.Model):
    ...
    exclude_from_review = fields.Boolean(
        string="Exclure de l'évaluation du vendeur", tracking=True, copy=False
    )
    ...
    def write(self, values):
        ...
        if "exclude_from_review" in values and not self._context.get(
            "ignore_exclude_from_review"
        ):
            (
                orders,
                invoices,
            ) = (
                self.line_ids.sale_line_ids.order_id.get_recursively_not_directly_related()
            )
            orders.with_context(
                ignore_exclude_from_review=True
            ).exclude_from_review = values["exclude_from_review"]
    ...
    def create(self, vals_list):
        ...
        for move in moves:
            (
                ...
                move.line_ids.sale_line_ids.order_id.get_recursively_not_directly_related()
            )
