...
class MailTemplate(models.Model):
    ...
    def generate_email(self, res_ids, fields):
        ...
        for record in (
            self.env[self.model]
            .browse(res_ids)
            .filtered(lambda r: r.bon_commande or r.bon_livraison)
        ):
            ...
            if record.bon_commande:
                new_attachments.append(
                    ("bon_commande-" + inv_print_name + ".pdf", record.bon_commande)
                )
            if record.bon_livraison:
                new_attachments.append(
                    ("bon_livraison-" + inv_print_name + ".pdf", record.bon_livraison)
                )
